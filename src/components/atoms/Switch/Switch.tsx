...
export const Switch = <OptionType extends { title: string; active: boolean }>({
  solid = false,
  vertical = false,
  options,
  onChange,
  ...
  return (
    ...
    >
      {options.map((option) => (
        <Option key={option.title} onClick={() => onChange(option)}>
          {option.title}
        </Option>
      ))}
    </Root>
...
const Root = styled.div<{ position: number; solid: boolean; count: number, vertical: boolean }>`
